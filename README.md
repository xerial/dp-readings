...
- [PrivateSQL: A Differentially Private SQL Query Engine] (2019) An early prototype to apply diffrential privacy to SQL processing. Although it only covers  `COUNT(*)` queries, it formalizes join processing between multiple tables (relations) and develop a method to split privacy budget inside pre-determined workloads.
